...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BRVMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BRVMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BRVMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("brv-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BRVMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running BRVMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadBRVMiner() exception");
    } catch (...) {
        LogPrintf("ThreadBRVMiner() exception");
    }

    LogPrintf("ThreadBRVMiner exiting\n");
}
